...
import { Plane, Upload, Clock, Calendar, Users, MapPin, ArrowRight, LayoutGrid, List, AlertCircle, FileText, ArrowUpRight, ArrowDownRight, AlertTriangle, Bug } from 'lucide-react';
import { generateStressData, generateEdgeCaseData } from './utils/testData';

// --- Sample Data from User PDF (Tweaked to show Overlap) ---
// Flight 2 changed to Pad 08L at 0935 to overlap with Flight 1 at 0930
// Added Flight 3 at 10:15 on a DIFFERENT pad (42C) to test multi-row overlap
...
const parseScheduleData = (text) => {
  ...
  const parseUnstructuredText = (fullText) => {
    // UPDATED Regex to capture Operator (col 6) and Crew (col 7)
    // Matches patterns like "ARK-1150" and "ARK - 1000" for times
...
const TooltipOverlay = ({ event, position }) => {
  ...
  if (event.type === 'conflict') {
    ...
    return (
      <div className="z-[9999] pointer-events-none" style={style}>
        <div className="bg-yellow-100 text-yellow-900 text-xs rounded-lg p-3 shadow-2xl border border-yellow-400 w-48 text-center">
          <div className="font-bold uppercase text-yellow-700 mb-1 border-b border-yellow-300 pb-1 flex items-center justify-center gap-2">
        ...
        <div className="w-3 h-3 bg-yellow-100 border-r border-b border-yellow-400 transform rotate-45 absolute bottom-0 left-1/2 -translate-x-1/2 translate-y-1.5"></div>
      </div>
    );
  }
  ...
  return (
    <div className="z-[9999] pointer-events-none" style={style}>
      <div className="w-64 bg-gray-900 text-white text-xs rounded-lg p-3 shadow-2xl border border-gray-700">
        ...
        <div className="space-y-1.5">
          <div className="flex justify-between">
            ...
            <span className="font-semibold text-blue-300">{event.flight.aircraft}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-gray-500">Route:</span>
            <span>{event.flight.from} ➝ {event.flight.to}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-gray-500">Pad Used:</span>
            <span>{event.pad}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-gray-500">Crew:</span>
            <span>{event.flight.crew}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-gray-500">Operator:</span>
            <span>{event.flight.operator}</span>
          </div>
          <div className="flex justify-between">
        ...
        <div className="mt-2 text-[10px] text-gray-500 text-right">
          Flight Duration: {event.flight.duration} min
        </div>
      </div>
      <div className="w-3 h-3 bg-gray-900 border-r border-b border-gray-700 transform rotate-45 absolute bottom-0 left-1/2 -translate-x-1/2 translate-y-1.5"></div>
    </div>
  );
};
...
const HubEventMarker = ({ event, style, onHover, onLeave }) => {
  ...
  return (
    <div
      className="absolute h-10 top-2 z-30 group" // Increased Z-index to sit on top of Conflict Layer (z-20)
      style={style}
      onMouseEnter={handleMouseEnter}
    ...
    >
      <div className={`w-full h-full rounded-md border ${bufferColor} relative flex items-center justify-center transition-all duration-200 ${localHover ? 'border-opacity-100 ring-1 ring-offset-1 ring-blue-300' : ''}`}>
        ...
        <div className={`z-20 ${iconColor} bg-white/80 rounded-full p-0.5 shadow-sm`}>
...
const Timeline = ({ flights, viewMode, onEventHover, onEventLeave }) => {
  ...
  // 1. First, process all events into a flat list for Global Congestion Check
  ...
  // 2. Calculate Global Conflicts (Time slices where active operations > 1)
  const globalConflicts = useMemo(() => {
    // Create a minute-by-minute timeline array
  ...
  return (
    <div className="w-full overflow-x-auto bg-slate-50 border border-slate-200 rounded-lg shadow-inner">
      <div className="min-w-[1000px] relative">
        {/* Header - Z-50 to stay on top of everything */}
        <div className="h-10 bg-slate-100 border-b border-slate-300 flex sticky top-0 z-50">
          <div className="w-32 flex-shrink-0 bg-slate-100 border-r border-slate-300 sticky left-0 z-50 flex items-center justify-center font-bold text-slate-600 text-sm shadow-sm">
          ...
          <div className="flex-grow relative">
            {hours.map(time => (
              <div
                key={time}
                className="absolute h-full border-l border-slate-300 text-xs text-slate-500 pl-1 pt-2 font-mono"
        ...
        {/* Global Conflict Overlay Layer - Z-20, sits on top of row backgrounds but below flights */}
        ...
        {/* Rows - Base layout, no explicit Z-index on container to allow layering */}
        <div className="divide-y divide-slate-200 relative">
          {rows.map(row => (
            <div key={row.id} className="h-14 flex group bg-white/50 hover:bg-slate-50/80 transition-colors relative">
              {/* Row Label - Z-40 to sit above conflicts */}
              <div className="w-32 flex-shrink-0 border-r border-slate-200 bg-white sticky left-0 z-40 flex items-center px-4 font-semibold text-slate-700 text-sm group-hover:bg-slate-50 shadow-[4px_0_5px_-2px_rgba(0,0,0,0.05)]">
                <div className={`w-2 h-8 rounded-full mr-3 ${viewMode === 'pad' ? 'bg-blue-500' : 'bg-purple-500'}`}></div>
                {row.label}
              </div>
              ...
              <div className="flex-grow relative">
                ...
                {hours.map(time => (
                  <div
                    key={time}
                    className="absolute h-full border-l border-slate-100"
                    style={{ left: `${((time - startOfDay) / totalDuration) * 100}%` }}
                  />
                ))}

                {/* Events - Z-30 to sit ON TOP of conflicts */}
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 font-sans relative">
      <header className="bg-white border-b border-slate-200 px-6 py-4 shadow-sm sticky top-0 z-50">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-slate-900 leading-tight">Hub Ops Visualizer (PDR)</h1>
              <p className="text-xs text-slate-500">Departure & Arrival Management</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4">
            <div className="bg-slate-100 p-1 rounded-lg flex text-sm font-medium">
              <button
                ...
                className={`px-4 py-1.5 rounded-md transition-all ${viewMode === 'pad' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
              >
                By Pad
              </button>
              <button
                onClick={() => setViewMode('aircraft')}
                className={`px-4 py-1.5 rounded-md transition-all ${viewMode === 'aircraft' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
              >
                By Aircraft
              </button>
            </div>
            ...
            {import.meta.env.DEV && (
              <button
                ...
                className={`p-2 rounded-lg transition-colors ${showDevTools ? 'bg-amber-100 text-amber-700' : 'text-slate-500 hover:bg-slate-100'}`}
                title="Dev Tools"
              >
                <Bug size={20} />
              </button>
      ...
      <main className="max-w-7xl mx-auto p-6">
        ...
        {showDevTools && (
          <div className="mb-6 bg-amber-50 p-4 rounded-xl border border-amber-200 animate-in slide-in-from-top-2">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <button
                onClick={() => setRawData('')}
                className="px-3 py-1.5 bg-slate-200 hover:bg-slate-300 text-slate-700 rounded text-xs font-semibold transition-colors"
              >
                Clear Data
              </button>
            </div>
        ...
        {showInput && (
          <div className="mb-6 bg-white p-6 rounded-xl shadow-md border border-slate-200 animate-in slide-in-from-top-4 duration-300">
            ...
            <textarea
              ...
              className="w-full h-48 font-mono text-xs p-4 bg-slate-50 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none resize-y"
              placeholder="Paste schedule data here..."
            />
            <div className="mt-4 flex justify-end gap-3 items-center">
              {isLoading && <span className="text-xs text-blue-600 animate-pulse font-medium">Processing PDF...</span>}
              <label className={`flex items-center gap-2 px-4 py-2 border border-slate-300 rounded-lg hover:bg-slate-50 cursor-pointer text-sm font-medium text-slate-600 transition-colors ${isLoading ? 'opacity-50 pointer-events-none' : ''}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 flex items-center gap-4">
            ...
            <div>
              <div className="text-2xl font-bold text-slate-800">{flights.length}</div>
              <div className="text-xs text-slate-500 uppercase tracking-wide font-semibold">Total Flights</div>
            </div>
          </div>
          ...
          <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 flex items-center gap-4">
            ...
            <div>
              ...
              <div className="text-xs text-slate-500 uppercase tracking-wide font-semibold">Hub Departures</div>
            </div>
          </div>
          ...
          <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 flex items-center gap-4">
            ...
            <div>
              ...
              <div className="text-xs text-slate-500 uppercase tracking-wide font-semibold">Hub Arrivals</div>
            </div>
          </div>
          ...
          <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 flex items-center gap-4">
            ...
            <div>
              ...
              <div className="text-xs text-slate-500 uppercase tracking-wide font-semibold">Personnel</div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-xl shadow-lg border border-slate-200 relative">
          <div className="flex justify-between items-center mb-6">
            <div>
              <h2 className="text-lg font-bold text-slate-800">Hub Operations (PDR)</h2>
          ...
          <div className="mt-6 flex gap-6 text-xs text-slate-500 border-t border-slate-100 pt-4">
